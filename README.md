...
Egocentric-network randomized trials (ENRTs) are a powerful design for
estimating causal effects in the presence of network interference.
However, the egocentric sampling process can lead to **ego-network contamination**,
where latent ego-ego or alter-ego edges are missing from the observed data.
...
You can install the development version of `ENRTsensitivity` from
[GitHub](https://github.com/barwein/ENRTsensitivity) with:
